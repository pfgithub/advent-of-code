...
(async () => {
	...
	board.set(cx + 1, cy + 1, "@");
	board.print();
	let startingBotPoses: { x: number; y: number }[] = [];
	board.forEach((v, x, y) => {
		if (v === "@") {
			startingBotPoses.push({ x, y });
			board.set(x, y, ".");
		}
	});
	console.log(startingBotPoses);
	// (((): any => {
	// 	throw -1;
	// }) as () => 0)();

	let paths: { doors: [string, number][]; distance: number }[] = [
		{
			doors: [],
			distance: 0,
		},
	...
	each bot{
		pathfind()
		each path{
			availablethingstodo.push(path)
		}
	}
	state says which robot does what
	so like a2b3c4d1
	determine if paths have been done before by:
	get all parts of path except last state of all robots, sort.
	get last state of all robots, sort
	
	so that path would be [],[a2b3c4d1]
	and if someone decides to do b3a2 it will already be done in a2b3
	*/
	while (true) {
		...
		let finalBotDoors: string[] = ["", "", "", ""];
		currentPath.doors.forEach(([door, bot]) => {
			finalBotDoors[bot] = door;
		});

		let nonfinalDoors = currentPath.doors.filter(
			m => finalBotDoors[m[1]] !== m[0],
		);
		...
		let allDoorsEmpty = true;
		for (let i = 0; i < 4; i++) {
			let cpos = cbotpos[i];
			let nextAvailableDoors = pathfind(pfb, cpos.x, cpos.y, 0);
			if (nextAvailableDoors.length === 0) continue;
			allDoorsEmpty = false;
			nextAvailableDoors.forEach(nad => {
				paths.push({
					doors: [...currentPath.doors, [nad.door, i]],
					distance: currentPath.distance + nad.distance,
				});
			});
		}
		if (allDoorsEmpty) {
			console.log("!!!done", currentPath.doors.map(d => d.join("")).join(","));
			console.log(currentPath.distance);
			return;
		}
		// add back doors
