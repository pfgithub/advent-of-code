...
(async () => {
	...
	*/
	...
	let fda = numbers.flatMap((n, i) => (i >= offset ? [n] : []));
	console.log(offset, numbers.length);

	for (let i = 0; i < 100; i++) {
		let pattern: number[] = [];
		let inarray = fda.reverse();
		let total = 0;
		inarray.forEach(item => {
			pattern.push((item + total) % 10);
			total += item;
		});
		fda = pattern.reverse();
		console.log(i, "/", 100, fda[0]);
	}
	console.log(fda.join("").substr(0, 8));
})();
