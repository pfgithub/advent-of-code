...
inv.forEach((wire, wireIndex) => {
	...
	wire.split`,`.forEach(segment => {
		...
		for (let i = 0; i < number; i++) {
			...
			let dw = dir === "R" || dir === "L" ? "-" : "|";
			let dw2 = "" + (wireIndex + 1);
			if (gv && gv[0].trim() && gv[0] !== dw2) {
				crossMarks.push([x, y, Math.min(stepCount + gv[1])]);
				qrwkjlnfdas = true;
			}
			set([x, y], [dw2, stepCount]);
			if (qrwkjlnfdas) {
				set([x, y], "X");
			}
		}
...
crossMarks.forEach(([x, y, stepCount]) => {
	lowestDistance = Math.min(lowestDistance, stepCount);
	console.log([x, y], stepCount);
});
